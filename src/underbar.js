(function() {
  ...
  _.last = function(array, n) {
	return n === undefined ? array[array.length-1] : array.slice(n<=array.length ? array.length-n : 0);
  };
  ...
  _.filter = function(collection, test) {
	  ...
	  _.each(collection, function(item){
		 if (test(item)) returned.push(item); 
	  });
	  return returned;
  };
  ...
  _.uniq = function(array) {
	  ...
	  _.each(array,function(item){
		  if(_.indexOf(unique,item)===-1) unique.push(item);
	  });
	  return unique;
  };
  ...
  _.extend = function(obj) {
	  	var leadObj = arguments[0];
		var otherObjs = Array.prototype.slice.call(arguments,1);
		
		_.each(otherObjs,function(otherObj){
  ...
  _.defaults = function(obj) {
		...
		_.each(otherObjs,function(otherObj){
			_.each(otherObj,function(value,key){
				if(!(key in leadObj)) leadObj[key] = value;
			});
		});
		return leadObj;
...
*/	
  ...
  _.invoke = function(collection, functionOrKey, args) {
		...
		return _.map(collection,function(item, index, arr){
  ...
  _.zip = function() {
		...
		var longest = _.sortBy(args.slice(), "length").reverse()[0].length;
		var zipped = [];
		
		_.each(args, function(argument){	
			for (var ziplength = 0; ziplength < longest; ziplength++){
				if (zipped[ziplength]===undefined) zipped[ziplength]=[];
				zipped[ziplength].push(argument[ziplength]);
			}
		});
		return zipped;
  };
